<template>
  <div
    ...
    :class="unsaved ? 'tw-border-brand-primary-light' : 'tw-border-gray'"
  >
    <div
...
export default {
  ...
  computed: {
    ...
    backgroundColor() {
      ...
      const maxAlpha = 255 - 30
      let alpha = Math.floor(frac * maxAlpha) / 255
      if (frac == 1) {
        alpha = 1
      }
      return `rgba(var(--color-brand-primary-rgb), ${alpha})`
    },
    fontColor() {
